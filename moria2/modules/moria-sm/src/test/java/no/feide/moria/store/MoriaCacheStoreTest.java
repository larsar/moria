...
public class MoriaCacheStoreTest extends TestCase {
    ...
    public void testCreateAuthnAttempt() throws InvalidTicketException {
        ...
        try {
            loginTicketId = store.createAuthnAttempt(null, prefix, postfix, principal, forceAuthn);
            fail("Null value for attributes did not cause exception to be thrown");
        } catch (IllegalArgumentException success) {
        }

        /* Emtpy string array should work */
        loginTicketId = store.createAuthnAttempt(new String[]{null}, prefix, postfix, principal, forceAuthn);
        store.getAuthnAttempt(loginTicketId, false);

        try {
            loginTicketId = store.createAuthnAttempt(attributes, null, postfix, principal, forceAuthn);
            fail("Null value for prefix did not cause exception to be thrown");
        } catch (IllegalArgumentException success) {
        }

        try {
            loginTicketId = store.createAuthnAttempt(attributes, prefix, null, principal, forceAuthn);
            fail("Null value for postfix did not cause exception to be thrown");
        } catch (IllegalArgumentException success) {
        }

        try {
            loginTicketId = store.createAuthnAttempt(attributes, prefix, postfix, null, forceAuthn);
    ...
    public void testCacheUserData() throws InvalidTicketException {
        ...
        MoriaTicket ticket = store.getTicket(store.cacheUserData(cachedAttrs));
        assertNotNull("SSO ticket should not be null", ticket);
        assertEquals("SSO ticket has wrong type", MoriaTicket.SSO_TICKET, ticket.getTicketType());
    }
    ...
    public void testServiceTicketCreation() throws InvalidTicketException {
        ...
        assertNotNull("Service ticket should not be null", store.getTicket(serviceTicketId));
        assertEquals("Service ticket has wrong type", MoriaTicket.SERVICE_TICKET, store.getTicket(serviceTicketId).getTicketType());
    }
    ...
    public void testGetUserData() throws InvalidTicketException {
        ...
        ssoTicketId = new MoriaTicket(MoriaTicket.SSO_TICKET, null, 30).getTicketId();
    ...
    public void testCreateTicketGrantingTicket() throws InvalidTicketException {
        ...
        MoriaTicket tgTicket = store.getTicket(tgTicketId);
    ...
    public void testCreateProxyTicket() throws InvalidTicketException {
        ...
        /* Invalid ticket */
        assertNull("Invalid ticket, should be null", store.createProxyTicket("doesNotExist", "foobar"));
        ...
        String ssoTicketId = store.cacheUserData(cachedAttrs);
        String tgTicketId = store.createTicketGrantingTicket(ssoTicketId, principal);
        String proxyTicketId = store.createProxyTicket(tgTicketId, principal2);

        assertNotNull("Proxy ticket should not be null", store.getTicket(proxyTicketId));
        assertEquals("Proxy ticket has wrong type", MoriaTicket.PROXY_TICKET, store.getTicket(proxyTicketId).getTicketType());
        assertEquals("Proxy ticket has wrong principal", principal2, store.getTicket(proxyTicketId).getServicePrincipal());

        /* Assert that tgt and proxy ticket references same userdata object */
        assertEquals("Userdata for TGT and Proxy ticket do not match", store.getUserData(tgTicketId), store.getUserData(proxyTicketId));
    }
    ...
    public void testSetTransientAttributes() throws InvalidTicketException {
        ...
        try {
            ...
            fail("IllegalStateException should be raised, ticket does not exist");
