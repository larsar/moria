...
public final class RequestUtil {
     ...
     * selected from the following priority list: URL parameter, cookie, service
     * config, browser setting, Moria default
     * @param bundleName
     *            name of the bundle to retrieve, cannot be null
     * @param requestParamLang
     *            language specified as URL parameter, can be null
     * @param langFromCookie
     *            language specified as cookie
     * @param serviceLang
     *            default language specified by service, can be null
     * @param browserLang
     *            language requested by the users browser, can be null
     * @param moriaLang
    ...
    public static ResourceBundle getBundle(final String bundleName, final String requestParamLang, final String langFromCookie, final String serviceLang, final String browserLang, final String moriaLang) {
        ...
        if (moriaLang == null || moriaLang.equals("")) { throw new IllegalArgumentException("moriaDefaultLang must be a non-empty string."); }
    ...
    public static Cookie createCookie(final String cookieName, final String cookieValue, final int validHours) {
        ...
        /* Validate parameters. */
        if (cookieName == null || cookieName.equals("")) { throw new IllegalArgumentException("cookieName must be a non-empty string."); }
        if (cookieValue == null || cookieValue.equals("")) { throw new IllegalArgumentException("cookieValue must be a non-empty string."); }
        if (validHours < 0) { throw new IllegalArgumentException("validDays must be a >= 0."); }
    ...
    /**
     * Reads institution names from the servlet config and generates a TreeMap
     * with the result.
     * @param config
     *            the web modules configuration
     * @param element
     *            the sub element of the configuration to process
     * @param language
     *            the language to generate institution names on
     * @return a TreeMap of institution names with full name as key and id as
     *         value object
     */
    static TreeMap parseConfig(final Properties config, final String element, final String language) {
        ...
        if (element == null || element.equals("")) { throw new IllegalArgumentException("element must be a non-empty string."); }
        ...
        if (value == null) { throw new IllegalStateException("No elements of type '" + element + "' in config."); }
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (index == -1) { throw new IllegalStateException("Config has wrong format."); }
            ...
            if (shortName.indexOf(":") != -1 || longName.indexOf(":") != -1) { throw new IllegalStateException("Config has wrong format."); }
