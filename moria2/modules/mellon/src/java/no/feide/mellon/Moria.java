 ...
 * service endpoint and attribute namespace URI. <br>
 * <br>
 ...
 * <br>
 * Note that this code is distributed 'as-is', for example purposes only. Use at
 * your own discretion.
 */
public class Moria {
     ...
     *            Moria's service endpoint. Must be a non-empty string.
     * @param serviceUsername
     *            The parent service's username, used when authenticating the
     *            service itself against Moria. Must be a non-empty string.
     * @param servicePassword
     *            The parent service's password, used when authenticating the
     *            service itself against Moria. Must be a non-empty string.
     * @param attributeNamespaceURI
     *            The attribute namespace URI, used for mapping the remote
     *            attribute class definition to the local
     *            <code>MoriaAttribute</code> definition. Must be a non-empty
     *            string.
     * @param trustStore
     *            The truststore file used when establishing trust with the
     *            Moria server. If <code>null</code> the default truststore is
     *            used.
     * @param trustStorePassword
     *            The password used to access the <code>trustStore</code>, if
     *            a truststore is used.
     */
    public Moria(final String serviceEndpoint, final String serviceUsername, final String servicePassword, final String attributeNamespaceURI, final String trustStore, final String trustStorePassword) {

        // Sanity checks
        if ((serviceEndpoint == null) || (serviceEndpoint.length() == 0))
            throw new IllegalArgumentException("Service endpoint must be a non-empty string");
        if ((serviceUsername == null) || (serviceUsername.length() == 0))
            throw new IllegalArgumentException("Service username must be a non-empty string");
        if ((servicePassword == null) || (servicePassword.length() == 0))
            throw new IllegalArgumentException("Service password must be a non-empty string");
        if ((attributeNamespaceURI == null) || (attributeNamespaceURI.length() == 0))
            throw new IllegalArgumentException("Attribute namespace must be a non-empty string");
    ...
    throws MalformedURLException, RemoteException {
        ...
        final Object[] parameters = {attributeRequest, urlPrefix, urlPostfix, new Boolean(false)};

        // Performing call.
        String s = (String) call.invoke(new QName("initiateAuthentication"), parameters);
        return s;
    ...
    /**
     * Gets user attributes following a successful user authentication. The
     * attributes have previously been requested through
    ...
    /**
     * Performs proxy authentication. Verifies, by using a proxy ticket
     * retrieved by <code>getProxyTicket</code>, that the user has been
     * authenticated and retrieves a number of attributes.
     * @param attributes
    ...
    throws MalformedURLException, RemoteException {
        ...
        // Perform the call.
        final Object returnedAttributes = call.invoke(new QName(myEndpoint, "proxyAuthentication"), parameters);
